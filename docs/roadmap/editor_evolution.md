...
**状态：** 已完成（M0 至 M8；M7 未触发并保持延期）

**更新日期：** 2026-07-21

**当前 package：** `vell`

**名称状态：** M3 已采用 `Vell`；`Eido` 未通过 M0 冲突检查
...
- 名称检查已排除 `Eido`，M3 已对 `Vell` 重复相同检查。
...
**记录：** [`m3_vell_rename.md`](m3_vell_rename.md)
...
1. `vell-protocol`：ID、几何、输入、Scene 和查询 DTO；
2. `vell-core`：Content、Buffer、文本事务和编辑领域逻辑；
3. `vell-frontend`：保持极小，只提供 Frontend 行为接缝；
4. `vell-mode`：只有 command、operation 和 presentation 契约已从 app
   编排细节中解耦后才提取；
5. `vell-app`：Kernel、Session、执行帧和生命周期协调；
6. `vell-plugin-v8`：ScriptHost、module、schema、bridge 和 worker；
7. `vell-tui`：现有 terminal 与 TUI 实现；
8. `vell`：只保留 CLI 和 composition root。

每次提取都必须移动真实代码并独立通过测试，不创建空壳 crate。
`vell-terminal` 暂不单独建立；只有第二个消费者真实复用 terminal
adapter 时再拆。`vell-plugin-api` 也暂缓，直到 schema 需要被非 V8
宿主复用或需要独立版本策略。
...
```text
                        vell
                    /      |       \
          vell-app  vell-tui  vell-plugin-v8
                |          |          |
                +----- vell-mode ---+
                |          |
          vell-core  vell-frontend
                 \        /
              vell-protocol
```
...
- `vell-protocol`、`vell-core` 和无脚本的 app 测试不编译 V8；
- `vell-app` 不依赖 crossterm、Taffy 或 V8；
- `vell-tui` 不依赖 `vell-app`；
- `vell-plugin-v8` 不向其他 crate 暴露 V8 类型；
- 每个提取提交可以独立 revert。
...
**状态：** 已评估，未触发（2026-07-21）

**记录：** [`m7_content_gate.md`](m7_content_gate.md)
...
**记录：** [`m8_release_quality.md`](m8_release_quality.md)
