...
**名称状态：** `Eido` 未通过 M0 冲突检查，正式名称待 M3 决定
...
1. `<brand>-protocol`：ID、几何、输入、Scene 和查询 DTO；
2. `<brand>-core`：Content、Buffer、文本事务和编辑领域逻辑；
3. `<brand>-frontend`：保持极小，只提供 Frontend 行为接缝；
4. `<brand>-mode`：只有 command、operation 和 presentation 契约已从 app
   编排细节中解耦后才提取；
5. `<brand>-app`：Kernel、Session、执行帧和生命周期协调；
6. `<brand>-plugin-v8`：ScriptHost、module、schema、bridge 和 worker；
7. `<brand>-tui`：现有 terminal 与 TUI 实现；
8. `<brand>`：只保留 CLI 和 composition root。

每次提取都必须移动真实代码并独立通过测试，不创建空壳 crate。
`<brand>-terminal` 暂不单独建立；只有第二个消费者真实复用 terminal
adapter 时再拆。`<brand>-plugin-api` 也暂缓，直到 schema 需要被非 V8
宿主复用或需要独立版本策略。
...
```text
                        <brand>
                    /      |       \
          <brand>-app  <brand>-tui  <brand>-plugin-v8
                |          |          |
                +----- <brand>-mode ---+
                |          |
          <brand>-core  <brand>-frontend
                 \        /
              <brand>-protocol
```
...
- `<brand>-protocol`、`<brand>-core` 和无脚本的 app 测试不编译 V8；
- `<brand>-app` 不依赖 crossterm、Taffy 或 V8；
- `<brand>-tui` 不依赖 `<brand>-app`；
- `<brand>-plugin-v8` 不向其他 crate 暴露 V8 类型；
- 每个提取提交可以独立 revert。
