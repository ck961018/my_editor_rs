...
**状态：** 所有权分层已完成（2026-07-13），多客户端调度待实际需求。

将共享内核状态与客户端状态分离：

- Kernel 共享 ContentStore、ModeRegistry 和后台服务；
- ClientSession 独立持有 Scene、Focus、ViewStore 和按键分发状态；
- 每个具体 Frontend 独立持有自己的 viewport 等 presentation 状态；
- 每个客户端通过自己的 transport 接收事件和查询数据。

如果远程场景始终只有一个连接，则不引入该层。

已提取 `Kernel` 与 `ClientSession`：App 只组合这两个所有权域和一个 Frontend。后台保存、
ContentStore 与 ModeRegistry 位于 Kernel；SceneBuilder、Scene revision、focus、ViewStore、
ViewId allocator 与 Dispatcher 位于 ClientSession。当前仍是单 session 事件循环，不引入
Arc/Mutex 或 session registry；测试验证两个 session 可共享 Kernel 内容而保持布局和
selection 独立。
