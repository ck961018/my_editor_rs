...
`docs/superpowers/specs/` 编写或更新 design spec；纯文件移动、测试拆分和不改变行为的
整理可以直接实施，但必须保持改动范围窄。
...
| 编号 | 优先级 | 状态 | 问题 |
| --- | --- | --- | --- |
| R01 | P0 | 待处理 | `app/mod.rs` 同时承担模块入口、核心实现和大型测试 |
| R02 | P0 | 待处理 | `Kernel` / `ClientSession` 只有字段分组，没有封装所属行为 |
| R03 | P0 | 待处理 | `ContentCommand` 分类与执行归属不一致，`Sequence` 可表达非法组合 |
...
第一项建议处理 **R01：整理 app 模块入口**。它主要是文件移动和测试组织，不要求先改变
命令模型或运行时语义，并能为 R02、R03 提供更清晰的实施位置。
