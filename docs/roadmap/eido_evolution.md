...
**当前 package：** `my_editor_rs`

**目标名称：** `Eido`（正式迁移前仍需完成名称可用性检查）
...
3. `my_editor_rs -> Eido` 先在单 crate 中完成；
...
5. 正式决定重命名前，检查 `eido` 在目标 registry、仓库托管平台和商标
   场景中的可用性。
...
- 首次读取旧路径时输出一次迁移诊断；
- TypeScript 中的 `editor` 是领域对象，不因品牌迁移改成 `eido`；
- 仓库托管平台重命名与源码提交分开执行。
...
1. `eido-protocol`：ID、几何、输入、Scene 和查询 DTO；
2. `eido-core`：Content、Buffer、文本事务和编辑领域逻辑；
3. `eido-frontend`：保持极小，只提供 Frontend 行为接缝；
4. `eido-mode`：只有 command、operation 和 presentation 契约已从 app
   编排细节中解耦后才提取；
5. `eido-app`：Kernel、Session、执行帧和生命周期协调；
6. `eido-plugin-v8`：ScriptHost、module、schema、bridge 和 worker；
7. `eido-tui`：现有 terminal 与 TUI 实现；
8. `eido`：只保留 CLI 和 composition root。

每次提取都必须移动真实代码并独立通过测试，不创建空壳 crate。
`eido-terminal` 暂不单独建立；只有第二个消费者真实复用 terminal adapter
时再拆。`eido-plugin-api` 也暂缓，直到 schema 需要被非 V8 宿主复用或需要
独立版本策略。
...
```text
                         eido
                    /      |       \
             eido-app  eido-tui  eido-plugin-v8
                |          |          |
                +------ eido-mode ----+
                |          |
             eido-core  eido-frontend
                 \        /
                eido-protocol
```
...
- `eido-protocol`、`eido-core` 和无脚本的 app 测试不编译 V8；
- `eido-app` 不依赖 crossterm、Taffy 或 V8；
- `eido-tui` 不依赖 `eido-app`；
- `eido-plugin-v8` 不向其他 crate 暴露 V8 类型；
- 每个提取提交可以独立 revert。
...
M0 baseline and decision gates
 ...
 -> M3 single-crate rename to Eido
