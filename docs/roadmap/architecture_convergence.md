...
**状态：** 进行中（R1 至 R5 已完成，R6 至 R10 待实施）

**更新日期：** 2026-07-21

**已完成基线验证：** `cargo fmt --check`、440 项测试、全目标 Clippy、
...
改造已经完成。当前架构以
[`editor-kernel-architecture.md`][1] 和源码为准；R1 至 R5 保留已完成的
收敛结论，R6 至 R10 记录下一轮扩展系统演进，不把未来设计写回当前实现
文档。
...
- `Content` 是封闭枚举，但 `ContentViewState` 仍用可选字段区分种类；
- Mode 可以动态注册，却不能声明自己适配哪些 Content；
- attach 只验证 ID 存在，不验证 Mode 与 Content 的组合是否合法；
- TypeScript 定义仍贴近宿主 callback，命令、键位和输入流语义不够直接。
...
普通 Mode API 稳定后，再把后台派生计算包装成单独高级能力。目标形态可以是：

```ts
analysis: {
  syntax: editor.background({
    input(ctx) { /* capture bounded snapshot */ },
    run(input) { /* worker computation */ },
    apply(ctx, result) { /* publish state/presentation */ },
  }),
}
```

具体 worker module 和 isolate 传输格式必须先写设计。宿主应自动处理 revision、
取消、旧结果丢弃、state transaction 和 presentation invalidation；普通 Mode
文档不出现 `job/applyJob/slot/includeText`。

在 mode-local 命令限定名稳定后，再评估 `editor.commands.define`。它应复用
同一 command registry，支持命令面板、重绑定、跨 Mode 调用和独立测试，而
不是另建一套脚本 action。没有实际调用入口前，不提前开放全局可变命令表。
...
R1 至 R5 已完成。R6 至 R9 每一阶段单独审查，先写对应设计和能够失败的架构
回归测试，再修改实现。R10 必须由真实插件用例驱动，不与基础 adapter 迁移
捆绑。Rust 代码阶段完成后运行：
