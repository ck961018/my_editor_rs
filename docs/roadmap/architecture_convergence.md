...
**状态：** 进行中（R1 至 R9 已完成，R10 待实施）

**更新日期：** 2026-07-21

**已完成基线验证：** `cargo fmt -- --check`、463 项测试、全目标 Clippy、
...
收敛结论，R10 记录剩余扩展系统演进，不把未来设计写回当前实现
文档。
...
- v1 脚本需要通过显式兼容层迁移到同一 registered adapter 形态。
- 后台分析仍需要与普通 command DSL 分层，避免增加基础 Mode 的
  认知成本。
