...
当前 `ViewData` 暂以 `Option<Selections>` 表达是否具有文本 selection。TUI 只在存在
selection 时执行文本 viewport 跟随、高亮和物理文本光标定位。完整 presentation enum
仍留给 roadmap 第六条，避免本项提前定义 Terminal/Web 的渲染协议。
