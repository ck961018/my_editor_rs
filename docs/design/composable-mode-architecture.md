...
- `InputExecutionFrame`：一次物理输入的原子执行范围；
...
嵌套 action 和 effect 共用一个有界执行预算。超出预算时当前
`InputExecutionFrame` 失败。
...
一次物理输入及其 Mode chain、effects、重放和嵌套命令属于同一个
`InputExecutionFrame`。
...
ModeContentState(ModeId, ContentId)
ModeViewState(ModeId, ViewId)
HistoryTransactionManager 的本次增量
```

快照在每个目标第一次发生可变访问前记录。Mode state 只参与执行失败恢复，
不进入 undo/redo 历史。
...
`InputExecutionFrame` 不是 `HistoryTransaction`。
...
呈现 callback 只读，不能修改状态或产生 effect：
...
TUI 先确定 viewport，再按可见文本范围拉取 decorations：

```text
TUI viewport
-> RenderQuery::decorations(view, visible_range)
-> AppQuery 按 ModeChain 查询
-> 合并、裁剪并解析 Face
-> TUI paint
```
