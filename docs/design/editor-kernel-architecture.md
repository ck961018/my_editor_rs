...
app 不借出或识别 `Buffer`、`StatusBar`；执行通过 `ContentStore` 分派。
Content 接收 `ContentAction`、保存请求和后台 `ContentEvent`，不接收顶层
`Command`、原始按键或可变 View state。
...
只读数据通过 `ContentStore::query` 返回 owned `ContentData`。Content 还
声明 `ContentPresentation::{Text, StatusBar}`，`AppQuery` 据此组装
...
脚本 primitive 直接创建 `OperationRequest`，旧 `ModeEffect` 只作为 v1
兼容 adapter 输入。
...
`(ModeId, ViewId)` 保存。layer 记录 source content/view revision，stale 或
faulted layer 不参与组合。policy 按 Mode 高到低取第一个显式值；decoration
按低到高组合，使高优先级后绘制。
...
- `ResolvedViewEdit` 仍作为旧 Mode API 的兼容 variant；
